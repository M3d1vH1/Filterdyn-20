...
class CommunicationAIAgent:
    ...
    def setup_gemini(self):
        """Initialize Gemini client for AI-powered content generation"""
        try:
    ...
    def generate_ai_email_content(self, context: Dict[str, Any]) -> str:
        """Generate email content using Gemini AI based on context"""
        if not self.gemini_model:
            return self._generate_fallback_content(context)
        try:
            prompt = self._build_email_prompt(context)
            system_prompt = "You are a professional business communication assistant for a water treatment company. Write clear, professional emails."
            response = self.gemini_model.generate_content([
                {"role": "user", "parts": [system_prompt]},
                {"role": "user", "parts": [prompt]}
            ],
            generation_config={
                "max_output_tokens": int(self.config.get('gemini_max_tokens', 1000)),
                "temperature": float(self.config.get('gemini_temperature', 0.7)),
                "top_p": float(self.config.get('gemini_top_p', 0.8)),
                "top_k": int(self.config.get('gemini_top_k', 40)),
            })
