...
class PlatformIntegrationConfig:
    ...
    GRANDSTREAM_CONFIG = {
        'base_url': os.getenv('GRANDSTREAM_URL', '') or '',
        'username': os.getenv('GRANDSTREAM_USERNAME', '') or '',
        'password': os.getenv('GRANDSTREAM_PASSWORD', '') or '',
    ...
    API_CORS_ORIGINS = (os.getenv('API_CORS_ORIGINS', '*') or '*').split(',')
    ...
    # External API Keys
    EXTERNAL_API_KEYS = {
        'service_1': os.getenv('EXTERNAL_API_KEY_1', '') or '',
        'service_2': os.getenv('EXTERNAL_API_KEY_2', '') or '',
        'service_3': os.getenv('EXTERNAL_API_KEY_3', '') or ''
    }
...
def get_config(environment: str = None) -> Dict[str, Any]:
    ...
    if environment is None:
        environment = os.getenv('FLASK_ENV', 'development') or 'development'
...
if __name__ == "__main__":
    ...
    print(f"Gemini API Key configured: {'Yes' if getattr(config['communication_ai'], 'GEMINI_API_KEY', None) else 'No'}") 